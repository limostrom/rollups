...
keep dealid dealdate companyid companyname ultimate_platform primaryindustrygroup primaryindustrycode
drop if ultimate_platform == ""
duplicates drop
...
bys ultimate_platform: egen modal_indcode = mode(primaryindustrycode)
...
collapse (count) deals = dealid_num (last) modal_indgrp modal_indcode ///
	...
	, by(ultimate_platform)
	
egen tot_deals = total(deals)
gsort -deals modal_indgrp ultimate_platform
...
replace cum_deal_share = deal_share + cum_deal_share[_n-1] if _n > 1
	replace cum_deal_share = int(cum_deal_share*1000)/10
gen sh_modal_ind = is_modal_indgrp / deals
	replace sh_modal_ind = int(sh_modal_ind*1000)/10
gen year_range = string(minyr) + "-" + string(maxyr)

// For Table top20.tex
order ultimate_platform deals cum_deal_share year_range modal_indgrp sh_modal_ind modal_indcode minyr maxyr
br ultimate_platform deals cum_deal_share year_range modal_indgrp sh_modal_ind if _n <= 20
...
graph export "output/bar_industry_platforms_ult_top1pct.pdf", replace as(pdf)
